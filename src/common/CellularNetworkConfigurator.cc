...
//
// Note: This extension would better be in the L3NetworkConfiguratorBase class
//       within the inet framework. We have opened a corrensponding pull request
//       (see https://github.com/inet-framework/inet/pull/753) - if it is merged,
//       the CellularNetworkConfigurator can be removed completely for newer INET
//       versions (>4.3.7).)

#include "CellularNetworkConfigurator.h"
...
Define_Module(CellularNetworkConfigurator);

void CellularNetworkConfigurator::initialize(int stage){
...
bool CellularNetworkConfigurator::isWirelessInterface(NetworkInterface *netIf)
{
    ...
    for(const auto& e : interfaceComparator){
        if (!strncmp(netIf->getInterfaceName(), e.first.c_str(), e.second)){
            return true;
        }
    }
    return false;
